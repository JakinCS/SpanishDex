...
export const getDashboardDeckInfo = unstable_cache(async (userId) => {
  ...
  try {
    ...
    await client.close();
    Sentry.captureException(error); // Capture the error event with Sentry
    revalidateTag('dashboard-page') // We don't want this error to be cached for 5 min... Invalidate the cache for this function.
    return {success: false, message: "Unexpected error occurred while loading the dashboard. Please try again.", error};
  }
}, ["dashboard-deck-info"], { tags: ['practice-info', 'deck-info', 'dashboard-page'], revalidate: 300 })

// This function retrieves deck information (and deck cards) based on the passed in deckId.  
export const getDeckInfo = unstable_cache(async (userId, deckId) => {
  ...
  try {
    ...
    await client.close();
    Sentry.captureException(error); // Capture the error event with Sentry
    revalidateTag('view-deck-page') // We don't want this error to be cached for 5 min... Invalidate the cache for this function.
...
}, ["get-deck-info"], { tags: ['practice-info', 'deck-info', 'view-deck-page'], revalidate: 300 })

// This function retrieves deck information (and deck cards) based on the passed in deckId.  
export const getEditDeckInfo = unstable_cache(async (userId, deckId) => {
...
}, ["get-edit-deck-info"], { tags: ['deck-info', 'edit-deck-page'], revalidate: 300 })
