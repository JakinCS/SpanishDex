...
import { isEmailValid, isPasswordValid, isUsernameValid, randomColorPair, revalidateCachingTag, spacedRepetitionCalculator } from "@/lib/utils";
...
export const createDeck = async (title, description, cards) => {
  ...
  try {
    ...
    revalidateCachingTag('deck-info') // Causes next.js to invalidate the cache of functions using the tag.
...
const getDashboardDeckInfoHelper = unstable_cache(async (userId) => {
  ...
  try {
    ...
    await client.close();
    Sentry.captureException(error); // Capture the error event with Sentry
    revalidateCachingTag('dashboard-page') // We don't want this error to be cached... Invalidate the cache for this function.
    return {success: false, message: "Unexpected error occurred while loading the dashboard. Please try again.", error};
  }
...
const getDeckInfoHelper = unstable_cache(async (userId, deckId) => {
  ...
  try {
    ...
    await client.close();
    Sentry.captureException(error); // Capture the error event with Sentry
    revalidateCachingTag('view-deck-page') // We don't want this error to be cached... Invalidate the cache for this function.
...
const getEditDeckInfoHelper = unstable_cache(async (userId, deckId) => {
  ...
  try {
    ...
    await client.close();
    Sentry.captureException(error); // Capture the error event with Sentry
    revalidateCachingTag('edit-deck-page') // We don't want this error to be cached for 5 min... Invalidate the cache for this function.
    return {success: false, message: "Unexpected error occurred while retrieving deck information. Please try again.", error};
  }
