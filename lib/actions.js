...
const createDeckHelper = async (client, userId, title, description, cards) => {
  ...
  try {
    ...
    cards.forEach((card) => {
      ...
      cardsToCreate.push({user: new ObjectId(userId), parent_deck: createDeck.insertedId, english: card.english, spanish: card.spanish, last_practiced: new Date(), next_practice_date: new Date(nextPracticeDate)});
    })
...
const editDeckHelper = async (client, deckId, userId, changes) => {
  ...
  try {
    ...
    let cardsToCreate = [];
    changes.addedCards.forEach((card) => {
      const nextPracticeDate = (new Date()).setDate((new Date()).getDate() + 1); // Set the next practice date to tomorrow
      cardsToCreate.push({user: new ObjectId(userId), parent_deck: new ObjectId(deckId), english: card.english, spanish: card.spanish, last_practiced: new Date(), next_practice_date: new Date(nextPracticeDate)});
    })
    ...
    if (cardsToDelete.length > 0) {
      ...
      if (deletedCards.deletedCount !== cardsToDelete.length) {
        console.log(deletedCards, cardsToDelete)
        await session.abortTransaction();
        await session.endSession();
    ...
    changes.otherCards.forEach(async (card) => {
      const updateObject = {}
      if (card.english !== undefined) updateObject.english = card.english;
      if (card.spanish !== undefined) updateObject.spanish = card.spanish;
