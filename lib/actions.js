...
export const resetPassword = async (username, password, token) => {
  ...
  try {
    ...
    await client.close();
    return { success: false, message: 'Password reset failed. Unexpected error occurred. Please try again later', error: error }
  }
}
...
export const sendResetPasswordMessage = async (email) => {
  // Define the client variable, holding a new MongoClient instance  
  const client = new MongoClient(process.env.MONGODB_URI);
  ...
  try {
    ...
    try {
      ...
      await client.close();
      return {success: false, error, message: "Email failed to send. Please try again"}
    }
    ...
    await client.close();
    return {success: false, error, message: "Unexpected error occurred. Please try again" }
  }
}