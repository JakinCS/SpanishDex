...
const createDeckHelper = async (client, userId, title, description, cards) => {
  ...
  try {
    ...
    for(let i = 0; i < cards.length; i++) {
      ...
      cardsToCreate.push({user: new ObjectId(userId), parent_deck: createDeck.insertedId, english: card.english, spanish: card.spanish, last_practiced: new Date(), next_practice_date: new Date(nextPracticeDate), sra: {n: 0, efactor: 2.5, interval: 1 / 48}});
    }
...
const editDeckHelper = async (client, deckId, userId, changes) => {
  ...
  try {
    ...
    for (let i = 0; i < changes.addedCards.length; i++) {
      ...
      const nextPracticeDate = (new Date()).setDate((new Date()).getDate() + 1); // Set the next practice date to tomorrow
      cardsToCreate.push({user: new ObjectId(userId), parent_deck: new ObjectId(deckId), english: card.english, spanish: card.spanish, last_practiced: new Date(), next_practice_date: new Date(nextPracticeDate), sra: {n: 0, efactor: 2.5, interval: 1 / 48}});
    }
