...
export const createAccount = async (username, email, password1, password2) => {
  ...
  try {
    ...
    const usernameFindResult = await collection.findOne({username: cleanUsername}, {projection: {username: 1}, collation: {locale: 'en_US', strength: 2}})
    ...
    if (cleanEmail !== '') {
      const emailFindResult = await collection.findOne({email: cleanEmail}, {projection: {email: 1}, collation: {locale: 'en_US', strength: 2}})
...
export const createDeck = async (title, description, cards) => {
  ...
  try {
    ...
    await revalidateCachingTag('deck-info') // Causes next.js to invalidate the cache of functions using the tag.
...
const createDeckHelper = async (client, userId, title, description, cards) => {
  ...
  try {
    ...
    for(let i = 0; i < cards.length; i++) {
      ...
      const nextPracticeDate = new Date((new Date().getTime() + (30 * 60 * 1000))); // Set the next practice date to 30 min from now
      cardsToCreate.push({user: new ObjectId(userId), parent_deck: createDeck.insertedId, english: card.english, spanish: card.spanish, last_practiced: new Date(), next_practice_date: new Date(nextPracticeDate), sra: {n: 1, efactor: 2.5, interval: 1 / 48}});
    }
...
export const deleteAccount = async (userId, password) => {
  ...
  try {
    ...
    const user = await collection.findOne({ _id: new ObjectId(userId) }, {projection: {password: 1}})
...
export const deleteDeck = async (deckId) => {
  ...
  try {
    ...
    const findDeck = await deckCollection.findOne({ _id: new ObjectId(deckId) }, {projection: {user: 1}})
...
export const editDeck = async (deckId, changes) => {
  ...
  try {
    ...
    await client.close();
    Sentry.captureException(error); // Capture the error event with Sentry
    return {success: false, message: 'Failed to save deck. Unexpected error occurred. Please try again later', error: error.toString()}
  }
}
...
const editDeckHelper = async (client, deckId, userId, changes) => {
  ...
  try {
    ...
    for (let i = 0; i < changes.addedCards.length; i++) {
      ...
      const nextPracticeDate = new Date((new Date().getTime() + (30 * 60 * 1000))); // Set the next practice date to 30 min from now
      cardsToCreate.push({user: new ObjectId(userId), parent_deck: new ObjectId(deckId), english: card.english, spanish: card.spanish, last_practiced: new Date(), next_practice_date: new Date(nextPracticeDate), sra: {n: 1, efactor: 2.5, interval: 1 / 48}});
    }
...
export const getDashboardDeckInfo = async () => {
  ...
  try {
    // Now that the session is verified, run this sensitive helper function and return the result.
    const result = await getDashboardDeckInfoHelper(session.user.id);
...
// This function handles all the sensitive query operations for the getDashboardDeckInfo function. 
// It isn't exported so that it's not accessible client-side.
const getDashboardDeckInfoHelper = unstable_cache(async (userId) => {
  ...
  try {
                ...
                '$add': [
                  '$$value', {
                    '$cond': [
                      {
                        '$lte': [
                          '$$this.next_practice_date', new Date()
                        ]
                      }, 1, 0
                    ]
                  }
    ...
    await client.close();
    Sentry.captureException(error); // Capture the error event with Sentry
    await revalidateCachingTag('dashboard-page') // We don't want this error to be cached... Invalidate the cache for this function.
    return {success: false, message: "Unexpected error occurred while loading the dashboard. Please try again.", error};
  }
}, ["dashboard-deck-info"], { tags: ['practice-info', 'deck-info', 'dashboard-page'], revalidate: 90 })

// This function retrieves deck information (and deck cards) based on the passed in deckId.  
export const getDeckInfo = async (deckId) => {

  // Check for the user being logged in.
  const session = await auth();
  if (!session?.user) return {success: false, message: "You are not logged in. Please log in to view this deck.", error: ''}

  try {
    // Now that the session is verified, run this sensitive helper function and return the result.
    const result = await getDeckInfoHelper(session.user.id, deckId);
...
// This function handles all the sensitive query operations for the getDeckInfo function. 
// It isn't exported so that it's not accessible client-side.
const getDeckInfoHelper = unstable_cache(async (userId, deckId) => {
  ...
  try {
    ...
    if (findDeck.user.toString() !== userId) {
      await client.close();
      return {success: false, message: 'Unauthorized. You are not logged in with the right account to view this deck.', error: ''};
    }
    ...
    await client.close();
    Sentry.captureException(error); // Capture the error event with Sentry
    await revalidateCachingTag('view-deck-page') // We don't want this error to be cached... Invalidate the cache for this function.
    return {success: false, message: "Unexpected error occurred while retrieving deck information. Please try again.", error: error.toString()};
  }
}, ["get-deck-info"], { tags: ['practice-info', 'deck-info', 'view-deck-page'], revalidate: 90 })

// This function retrieves deck information (and deck cards) based on the passed in deckId.  
export const getEditDeckInfo = async (deckId) => {
  ...
  try {
    // Now that the session is verified, run this sensitive helper function and return the result.
    const result = await getEditDeckInfoHelper(session.user.id, deckId);
...
// This function handles all the sensitive query operations for the getEditDeckInfo function. 
// It isn't exported so that it's not accessible client-side.
const getEditDeckInfoHelper = unstable_cache(async (userId, deckId) => {
  ...
  try {
    ...
    if (findDeck.user.toString() !== userId) {
      await client.close();
      return {success: false, message: 'Unauthorized. You are not logged in with the right account to edit this deck.', error: ''};
    }
    ...
    await client.close();
    Sentry.captureException(error); // Capture the error event with Sentry
    await revalidateCachingTag('edit-deck-page') // We don't want this error to be cached for 5 min... Invalidate the cache for this function.
    return {success: false, message: "Unexpected error occurred while retrieving deck information. Please try again.", error};
  }

}, ["get-edit-deck-info"], { tags: ['deck-info', 'edit-deck-page'], revalidate: 90 })
...
export const resetPassword = async (username, password, token) => {
  ...
  try {
    ...
    const user = await collection.findOne({username: cleanUsername, reset_token: hashedToken, reset_token_expiry: {$gt: Date.now()}}, {projection: {username: 1}, collation: {locale: 'en_US', strength: 2}})
    if (!user) {
      await client.close();
...
// This function is used to run the revalidateTag function.
// (helpful for keeping that code separate from the cached functions themselves)
export const revalidateCachingTag = async (tag) => {
  revalidateTag(tag)
}
...
export const sendResetPasswordMessage = async (prevState, fieldValues) => {
  ...
  try {
    ...
    let searchResult = await collection.findOne({email: cleanEmail}, {projection: {email: 1}, collation: {locale: 'en_US', strength: 2}});
    ...
    const updateResult = await collection.updateOne({email: cleanEmail}, { $set: {reset_token: databaseResetToken, reset_token_expiry: tokenExpirationDate}})
    ...
    try {
      ...
      const removeResult = await collection.updateOne({email: cleanEmail}, { $set: {reset_token: undefined, reset_token_expiry: undefined}})
