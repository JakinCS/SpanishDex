...
// }
export const handlePasswordValidCheck = (field1Info, field2Info) => {

  // Check whether the format of the password is valid.
  const passwordValid = isPasswordValid(field1Info.value);

  if (!passwordValid.valid) {
    // If it isn't valid, return the error info of the field. No other logic needs to run.
    return [{...field1Info, valid: passwordValid.valid, errorType: 'reg', message: passwordValid.message}, field2Info];
  }

  // check to see if the two fields are matching in value
  const matchesOtherField = (field1Info.value === field2Info.value);

  /* If the fields match, don't indicate a 'matching error' and return that this field is valid. 
  ...
  if (matchesOtherField || (!field2Info.valid && field2Info.errorType === 'reg') || field2Info.valid === null) {
    // No errors for this field. Indicate so in the return value
    const returnInfo = [{...field1Info, valid: true, errorType: ''}, field2Info]

    // If the other field currently displays a "matching error", it will need to be removed because the fields now match.
    if (!field2Info.valid && field2Info.errorType === 'matching') {
      returnInfo[1] = {...field2Info, valid: true, errorType: ''}
    }

    return returnInfo
  } else { // Otherwise, this field needs a 'matching error'. So return that information.
    const returnInfo = [{...field1Info, valid: false, errorType: 'matching', message: 'Passwords must match'}, field2Info]

    // If the other field is current showing to be valid, it needs a matching error too.
    if (field2Info.valid) {
      returnInfo[1] = {...field2Info, valid: false, errorType: 'matching', message: 'Passwords must match'}
    }
...
export const revalidateCachingTag = (tag) => {
  revalidateTag(tag)
}


// This function randomly shuffles the passed in array.
