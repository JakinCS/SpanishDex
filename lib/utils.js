...
export const handlePasswordValidCheck2 = (field1Info, field2Info, formStateSetter, fieldNumber) => {
  // let mainPasswordField;
  // let otherPasswordField
  // if (fieldNumber === 1) {
  //   mainPasswordField = "password";
  ...
  let field1ReturnInfo = {...field1Info};
  let field2ReturnInfo = {...field2Info};
  ...
  if (!passwordValid.valid) {
    // If it isn't valid, display an error. No other logic needs to run.
    // formStateSetter(prevState => ({...prevState, [mainPasswordField]: {...prevState[mainPasswordField], value: field1Info.value, valid: passwordValid.valid, errorType: 'reg', message: passwordValid.message}}))
